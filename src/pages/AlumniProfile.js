import React from 'react';
import './AlumniProfile.css'; // Import the CSS file


function AlumniProfiles(){
    return(
        <div className='flex justify-center mt-10 '>
            <div className="card card-image-cover mr-10 ">
	<img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRcMqe3iztc7zMqug9EQdggPLtlDvjzUcv8cA&usqp=CAU "  alt="" />
	<div className="card-body">
		<h2 className="card-header">PETER</h2>
		<p className="text-content2">SOFTWARE DEVELOPER AT AMAZON</p>
        <p className="text-content2"> contactEmail: 'Peter@gmail.com'</p>
        <a className="link" href="https://www.linkedin.com/in/shaik-navrin-988549294">Go to linkedin</a>
	</div>
</div>
<div className="card card-image-cover mr-10">
	<img src="data:image/jpeg;base64," alt="" />
	<div className="card-body">
		<h2 className="card-header">JOHN</h2>
		<p className="text-content2">JPMC</p>
        <p className="text-content2"> contactEmail: 'John@gmail.com'</p>
        <a className="link" href="https://www.linkedin.com/in/shaik-navrin-988549294">Go to linkedin</a>
	</div>
</div>
<div className="card card-image-cover">
	<img src="https://t4.ftcdn.net/jpg/02/24/86/95/360_F_224869519_aRaeLneqALfPNBzg0xxMZXghtvBXkfIA.jpg" alt="" />
	<div className="card-body">
		<h2 className="card-header">SMITH</h2>
		<p className="text-content2">GOOGLE</p>
        <p className="text-content2"> contactEmail: 'John@gmail.com'</p>
        <a className="link" href="https://www.linkedin.com/in/shaik-navrin-988549294">Go to linkedin</a>
	</div>
</div>
        </div>
    )
}
export default AlumniProfiles;